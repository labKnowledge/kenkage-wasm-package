...
static const char *DOM_PRELUDE =
    ...
    "    if (!hasScheme) {\n"
    "      if (base === undefined) {\n"
    "        // Framework bundles sometimes call new URL(assetPath) after\n"
    "        // obtaining a relative currentScript.src. A browser exposes\n"
    "        // that src as absolute; use the active page as the equivalent\n"
    "        // implicit base in this sandbox.\n"
    "        base = globalThis.location && globalThis.location.href !== 'about:blank'\n"
    ...
    "};\n"
    "globalThis.__kk_URL = globalThis.URL;\n"
    ...
    "});\n"
    "Object.defineProperty(__LPWNodeProto, 'src', {\n"
    ...
    "  }\n"
    /* Script nodes are used as a concrete DOM type by Next.js' bootstrap
     * runtime. Their wrappers still need the generic node methods, so keep
     * the native node prototype below HTMLScriptElement.prototype. */
    "  Object.setPrototypeOf(globalThis.HTMLScriptElement.prototype, __LPWNodeProto);\n"
    "})();\n"
    ...
    "        if (this.__pendingReads.length) this.__pendingReads.shift()({ done: false, value: chunk });\n"
    "        else this.__chunks.push(chunk);\n"
    "      },\n"
    "      close: () => {\n"
    "        if (this.__closed) return;\n"
    "        this.__closed = true;\n"
    "        while (this.__pendingReads.length) this.__pendingReads.shift()({ done: true, value: undefined });\n"
    "      },\n"
    "      error: (e) => {\n"
    "        if (this.__closed) return;\n"
    "        this.__closed = true; this.__error = e;\n"
    "        while (this.__pendingReads.length) this.__pendingReads.shift()(Promise.reject(e));\n"
    "      },\n"
    "    };\n"
    ...
    "        if (self.__closed) return Promise.resolve({ done: true, value: undefined });\n"
    "        return new Promise((resolve, reject) => {\n"
    "          self.__pendingReads.push((result) => {\n"
